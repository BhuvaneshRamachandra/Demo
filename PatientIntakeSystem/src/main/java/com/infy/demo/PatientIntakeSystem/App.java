...
public class App 
{
    ...
    private static void performPatientEntry(Scanner scanner) {
        ...
        try {
            calendar.addAppontments(firstName, lastName, doc, when);
        } catch (Throwable t) {
    ...
    private static void performAllAppointments() throws Throwable {
        ...
        for (Appointment appointment : calendar.getListofAppointments()) {
            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd/MM/yyyy hh:mm a");
            String apptTime = formatter.format(appointment.getAppointmentdate());

            System.out.println(String.format("%s: %s, %s\t\tDoctor: %s", apptTime, appointment.getPatineLastName(),
                    appointment.getPatineFirstName(), appointment.getDocotr().getName()));
        }
